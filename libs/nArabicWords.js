...
exports.nArabicWords = function (
	...
	if (IsSubject) {
		...
		// Subject name with Tanween for 11-99
		else if (Triplet > 2) SubjectName = space + Subject[2];
		// Subject name Plural for 3-10
		else if (Triplet > 0)
	...
	function oneTripletToWords() {
		...
		if (Num_100) {
			...
			if (Num_100 > 2)
				Word_100 = TableFemale[Num_100] + (SplitHund === ON ? ' ' : '') + Miah;
			// 300-900
			else if (Num_100 === 1) Word_100 = Miah;
			// 100
			else
				Word_100 =
					Miah.slice(0, -1) + ((Scale && !Num_99) || TextToFollow ? Taa : Taan); // 200 Use either مئتا or مئتان
		}
		...
		if (Scale) {
			...
			if (Num_99 > 2) {
				...
				// Scale for 1
					...
					Words999 = Word_100Wa + (IsLastEffTriplet && TextToFollow ? Aa : Aan); // Scale for 2 ألفا or ألفان
			}
		}
